...
def tcp_server(port):
    ...
    logging.debug (f"TCP Server listening on {host}:{port}")
    while True:
...
def udp_server(port):
    ...
    logging.debug (f"UDP Server listening on {host}:{port}")
    while True:
        data, client_address = udp_socket.recvfrom(1024)
        logging.info (f"New UDP connection from {client_address}")
        logging.debug (f"Received data from {client_address}: {data}")
